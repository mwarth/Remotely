...
if ($SignAssemblies) {
    &"$Root\Utilities\signtool.exe" sign /f "$CertificatePath" /p $CertificatePassword /tr http://rfc3161timestamp.globalsign.com/advanced /td SHA256 "$Root\Server\wwwroot\Content\Win-x64\Remotely_Desktop.exe"
    &"$Root\Utilities\signtool.exe" sign /f "$CertificatePath" /p $CertificatePassword /as /fd sha256 /tr http://rfc3161timestamp.globalsign.com/advanced /td SHA256 "$Root\Server\wwwroot\Content\Win-x64\Remotely_Desktop.exe"
}
...
if ($SignAssemblies) {
    &"$Root\Utilities\signtool.exe" sign /f "$CertificatePath" /p $CertificatePassword /tr http://rfc3161timestamp.globalsign.com/advanced /td SHA256 "$Root\Server\wwwroot\Content\Win-x86\Remotely_Desktop.exe"
    &"$Root\Utilities\signtool.exe" sign /f "$CertificatePath" /p $CertificatePassword /as /fd sha256 /tr http://rfc3161timestamp.globalsign.com/advanced /td SHA256 "$Root\Server\wwwroot\Content\Win-x86\Remotely_Desktop.exe"
}
...
if ($SignAssemblies) {
    &"$Root\Utilities\signtool.exe" sign /f "$CertificatePath" /p $CertificatePassword /tr http://rfc3161timestamp.globalsign.com/advanced /td SHA256 "$Root\Server\wwwroot\Content\Remotely_Installer.exe"
    &"$Root\Utilities\signtool.exe" sign /f "$CertificatePath" /p $CertificatePassword /as /fd sha256 /tr http://rfc3161timestamp.globalsign.com/advanced /td SHA256 "$Root\Server\wwwroot\Content\Remotely_Installer.exe"
}
